...
export default function QuizStart({
  ...
  const [answers, setAnswers] = useState(Array(maxQuestions).fill(null));
  ...
  // Quiz sorularını her başlatmada rastgele seç
  const [quizQuestions, setQuizQuestions] = useState(() => getRandomQuestions(questions, maxQuestions));
  ...
  // Parabola grafiği
  const graphData = (() => {
    if (!question || !question.graph || !question.graph.function) return null;

    const xLabels = [...Array(51).keys()].map((i) => i / 10);
    let fn = question.graph.function.replace("y =", "").trim();

    const dataPoints = xLabels.map((x) => {
      let f = fn.replace(/(\d)(x)/g, "$1*x");
      f = f.replace(/x\^2/g, "Math.pow(x,2)");
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-900 text-center p-8">
        ...
        <p className="text-lg mb-6">
          Skorun: <span className="font-bold">{correctCount}</span> / {quizQuestions.length} (
          {(rate * 100).toFixed(0)}%)
        </p>
        ...
        {passed ? (
          <Link
            ...
            className="px-6 py-3 bg-red-500 text-white rounded-xl shadow-md hover:scale-105 transition-transform"
          >
            Hard Quiz’e Geç
          </Link>
        ) : (
          <Link
            ...
            className="px-6 py-3 bg-gray-500 text-white rounded-xl shadow-md hover:scale-105 transition-transform"
          >
            Konuyu Tekrar Et
          </Link>
        )}
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 w-full max-w-2xl">
          ...
          <div className="bg-gray-100 dark:bg-gray-700 p-3 rounded-xl shadow mb-6">
            <Line
              ...
              options={{
                responsive: true,
                scales: {
                  x: { type: "linear", min: graphData.xMin, max: graphData.xMax, ticks: { stepSize: 1 } },
                  y: { type: "linear", min: graphData.yMin, max: graphData.yMax, ticks: { stepSize: 1 } },
                },
        ...
        <div className="mt-6 flex justify-between w-full">
          <button
            ...
            className="px-4 py-2 bg-gray-400 text-white rounded-xl shadow-md hover:scale-105 transition-transform"
          >
            Boş Geç
          </button>
          ...
          <button
            ...
            className="px-6 py-2 bg-gradient-to-r from-blue-500 to-purple-500 text-white rounded-xl shadow-md hover:scale-105 transition-transform disabled:opacity-50"
          >
...
export async function getServerSideProps({ params }) {
  ...
  const quizPath = path.join(process.cwd(), "content", "exams", exam, topic, subtopic, "quiz.json");
  let questions = [];
  ...
  if (fs.existsSync(quizPath)) {
    ...
    questions = [
      {
        question: "Henüz bu konu için quiz hazırlanmadı.",
        options: ["-"],
        answer: 0,
      },
    ];
  ...
  return {
    props: { exam, topic, subtopic, questions },
  };
}
